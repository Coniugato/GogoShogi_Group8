...
//0の時は通常、1の時は盤面を表示する。
static int Display_Board_Enabled=1;
//盤及び持ち駒による表示。
...
static char possessed[2][12]={{'x','x','x','x','x','x','x','x','x','x','x','x'},
...
static AVL_TREE_Node* Board_Memorizing_AVL_Tree[2]={NULL, NULL};

char* Initialize_Game(){
    Board_Memorizing_AVL_Tree[0]=NULL;
    Board_Memorizing_AVL_Tree[1]=NULL;
    char initial_board[5][5][2]={
        {{'H', 'D'},{'A', 'D'},{'S','D'},{'K', 'D'},{'G', 'D'}},
        {{'x', 'x'},{'x', 'x'},{'x', 'x'},{'x', 'x'},{'F', 'D'}},
        {{'x', 'x'},{'x', 'x'},{'x', 'x'},{'x', 'x'},{'x', 'x'}},
        {{'F', 'U'},{'x', 'x'},{'x', 'x'},{'x', 'x'},{'x', 'x'}},
    ...
    char initial_possessed[2][12]={{'x','x','x','x','x','x','x','x','x','x','x','x'},
        {'x','x','x','x','x','x','x','x','x','x','x','x'}};
    int i, j;
    for(i=0; i<5; i++) for(j=0; j<5; j++){
        board[i][j][0]=initial_board[i][j][0];
        board[i][j][1]=initial_board[i][j][1];
    }
    for(i=0; i<2; i++) for(j=0; j<12; j++){
        possessed[i][j]=initial_possessed[i][j];
    }
    Num_Moves=0;
    Process_Sen_Nichi_Te(1,0);
}
...
char* Get_Info(){
    ...
    int i, j;
    for(i=0; i<5; i++) for(j=0; j<5; j++){
        info[num]=board[i][j][0];
        num++;
        info[num]=board[i][j][1];
        num++;
    }
    for(i=0; i<2; i++) for(j=0; j<12; j++){
        info[num]=possessed[i][j];
        num++;
    }
    info[num]='\0';
    return info;
...
//外部操作の場合の手番の処理
int Controlled_Process(int side, char* input){
    printf("%s\n", input);
...
//勝った場合は3、負けた場合は4を返す。引き分けは5。次の手番を返す
int game_proceed(int side, char* instruction){
    //今の盤面になった回数を保持する変数
    int Num_This_Board;
    int Flag=Controlled_Process(side, instruction);
    if(Flag==0) return 4;
    if(isOte(1-side)){
        if(Display_Board_Enabled) Display_Board(1-side);
        return 4;
    }
    Num_This_Board=Process_Sen_Nichi_Te(1, 1-side);
    if(isTsumi(1-side)){
        //打った結果相手が詰んだ
        if(Display_Board_Enabled) Display_Board(1-side);
        return 3;
...
//盤面の状態(board_cur,possessed_cur,side)をもらって整数を返す評価関数
//とりあえず常に1を返す
int evaluate_Fanction(char board_cur[5][5][2], char possessed_cur[2][12], int side, int my_side){
    int i, j, k, score=0;
    char mv;
    for(i=0; i<5; i++) for(j=0; j<5; j++) for(k=0; k<2; k++){
        mv=board_cur[i][j][k]; board_cur[i][j][k]=board[i][j][k]; board[i][j][k]=mv;
    }
    for(i=0; i<2; i++) for(j=0; j<12; j++){
        mv=possessed_cur[i][j]; possessed_cur[i][j]=possessed[i][j]; possessed[i][j]=mv;
    }
    if(side!=my_side){
        if(isOte(side)==-1){score=-1000000;}
        else if(isOte(my_side)==-1){score=10000000;}
        else if(Process_Sen_Nichi_Te(0,side)>=4 && my_side==0){score=-100000;}
        else if(Process_Sen_Nichi_Te(0,side)>=4 && isOte(my_side)){score=-100000;}
        else if(isTsumi(side)){score=100000;}
        else if(isOte(side)){score=-100000;}
    }
    //else{
        //if(Process_Sen_Nichi_Te(0,side)>=4 && isOte(side)){score=100000;}
    //}
    for(i=0; i<5; i++) for(j=0; j<5; j++) for(k=0; k<2; k++){
        mv=board_cur[i][j][k]; board_cur[i][j][k]=board[i][j][k]; board[i][j][k]=mv;
    }
    ...
    Py_Initialize();
    set_path();
    pName = PyUnicode_DecodeFSDefault("Forward");
    pModule = PyImport_Import(pName);
    if(pModule != NULL){
    pFunc = PyObject_GetAttrString(pModule, "forward");//関数名
    }
    char board_str[100]="";
    char possessed_str[50]="";
    PyObject *pValue;
    

    for(i=0; i<5; i++) for(j=0; j<5; j++) for(k=0; k<2; k++) board_str[i*10+j*2+k]=board_cur[i][j][k];
    for(i=0; i<5; i++) for(j=0; j<5; j++) possessed_str[i*12+j]=possessed_cur[i][j];
    board_str[50]='\0';
    possessed_str[24]='\0';
    PyObject *pArgs = Py_BuildValue("ssii", board_str, possessed_str, side, my_side);
...
char* Alpha_Beta(int side){
    ...
    int max = -999999999;
    int inst_index;
    int depth = 2; //探索の深さ(試した感じ10秒以内では8が限界っぽい？)
    char *return_inst;
    char next_board[5][5][2];
    char next_possessed[2][12];
    for(int n = 0; n < inst_count; n++){
